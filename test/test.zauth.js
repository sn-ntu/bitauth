...
describe('zauth', function() {
  ...
  describe('#getPublicKeyFromPrivateKey', function() {
    ...
    it('should properly get the public key', function(done) {
      zauth.getPublicKeyFromPrivateKey(keys.priv).should.equal(keys.pub);
      done();
    });

    it('should properly get compressed public key from a previously known private key', function(done) {
      zauth.getPublicKeyFromPrivateKey(keysKnown.priv).should.equal(keysKnown.pub);
      done();
    });
  ...
  describe('#getSinFromPublicKey', function() {
    ...
    it('should properly get the sin', function(done) {
      zauth.getSinFromPublicKey(keys.pub).should.equal(keys.sin);
      done();
    });

    it('should properly get the sin from a previously known compressed public key', function(done) {
      zauth.getSinFromPublicKey(keysKnown.pub).should.equal(keysKnown.sin);
      done();
    });
  ...
  describe('#sign', function() {
    ...
    it('should sign the string', function(done) {
      signature = zauth.sign(contract, keys.priv);
  ...
  describe('#verifySignature', function() {
    ...
    it('should verify the signature', function(done) {
      zauth.verifySignature(contract, keys.pub, signature, function(err, valid) {
    ...
    it('should verify the signature with leading zero public key', function(done) {
      ...
      var leadingZeroKeys = {
        ...
        pub: zauth.getPublicKeyFromPrivateKey(privateKeyToZero)
      };

      signature = zauth.sign(contract, leadingZeroKeys.priv);
      zauth.verifySignature(contract, leadingZeroKeys.pub, signature, function(err, valid) {
        should.not.exist(err);
        should.exist(valid);
        valid.should.equal(true);
      });
    ...
    describe('Reference Signature Tests', function () {
        ...
        var pub = zauth.getPublicKeyFromPrivateKey(priv);
        var refPairs = [
            ["foo",
        ...
        refPairs.forEach(function (pair) {
            ...
            it('should verify reference signature for: "' + contract + '"', function (done) {
                zauth.verifySignature(contract, pub, signature, function (err, valid) {
                    should.not.exist(err);
                    should.exist(valid);
                    valid.should.equal(true);
                    done();
  ...
  describe('#validateSinTrue', function() {
    ...
    it('should validate the sin as true', function(done) {
      var valid = zauth.validateSin(singood);
  ...
  describe('#validateSinCallback', function() {
    ...
    it('should receive error callback', function(done) {
      zauth.validateSin(sinbad, function(err) {
        should.exist(err);
  ...
  if (typeof(window) === 'undefined') {
    ...
    describe('#encrypt', function() {
      ...
      it('should encrypt the secret message', function(done) {
        enc = zauth.encrypt(password, secret);
        should.exist(enc);
        done();
      });
    });
    ...
    describe('#decrypt', function() {
      ...
      it('should decrypt the secret message', function(done) {
        var dec = zauth.decrypt(password, enc);
        should.exist(dec);
      ...
      it('should decrypt a previously known message', function(done) {
        var dec = zauth.decrypt(password, encryptedSecret);
        should.exist(dec);
        dec.should.equal(secret);
        done();
      });
